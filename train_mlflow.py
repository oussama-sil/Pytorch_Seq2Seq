...
def train(model,data,loss_fct,optimizer,nb_epochs,device,tracking_params,eval_fct,checkpoint_options,tensorboard_options,exec_options,clip = 1,transformer=False):
    ...
    mlflow.log_param("Batch_nb", nb_batch)
    mlflow.log_param("Datasets", ['hello','hello'])
    ...
    for epoch in range(nb_epochs):
        ...
        for i, (x_ind_pad,y_ind_pad,lengths_x,lengths_y,x,y,x_token,y_token) in enumerate(train_loader):
            ...
            mlflow.log_metric("Loss", loss.detach().item(), step=epoch*nb_batch+i)
            mlflow.log_metric("PPL", loss.detach().item(), step=epoch*nb_batch+i)
...
def work():
    ...
    current_dir = os.path.join(os.getcwd(),'mlruns',)
    mlflow.set_tracking_uri(uri=f"file://localhost/{current_dir}") # Logging to local file 

    mlflow.set_experiment("Seq2SeqTransformer")
    mlflow.start_run(run_name= '8')
    mlflow.set_tag("Key_Tag", "Test Exp")    
    ...
    valid_loader = DataLoader(dataset=valid_dataset,batch_size=8,shuffle=False,collate_fn=collate_fn)
    ...
    #? Optimizer 
    optimizer = torch.optim.Adam(seq2seq.parameters(),lr=2e-4)
